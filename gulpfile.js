...
// constante que coleta parametros no terminal, gulp teste --a oi (a é key e oi é o value)
const arg = ((argList) => {
    let arg = {},
        a,
        opt,
...
//testando parametro no terminal digite -> gulp testeParametro --nome Livio
gulp.task("testeParametro", (done) => {
    console.log("arg = " + arg.nome);
    done();
});
...
// Apagando a pasta assets/css/components
gulp.task("clean-css-components", function () {
    return gulp
        .src(`${dir}assets/css/components/`, { read: false, allowEmpty: true })
        .pipe($.clean());
});
...
gulp.task("copy-vcf", function () {
    return gulp.src(["site/assets/files/*.vcf"]).pipe(gulp.dest(`${dir}assets/files/`));
});
...
gulp.task(
    ...
    gulp.series(
        "clean",
        gulp.parallel("copy-font", "copy-vcf", "copy-robots"),
        "imagemin",
        "separaCss",
